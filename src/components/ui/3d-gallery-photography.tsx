...
function GalleryScene({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent) => {
      // If animation is complete (3 loops done), allow normal scroll
    ...
    [speed, animationComplete],
  );

  // Handle keyboard input
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      // If animation is complete, don't handle keyboard
  ...
  useEffect(() => {
    ...
    const handleVisibilityReset = (event: CustomEvent) => {
    ...
    window.addEventListener(
      "resetGalleryAnimation" as any,
      handleVisibilityReset as any,
    );
    return () =>
  ...
  useFrame((state, delta) => {
    // Stop animation if 3 loops complete
    if (animationComplete) {
      setScrollVelocity(0);
      return;
    }
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && entry.intersectionRatio > 0.5) {
            // User scrolled back to hero section, dispatch reset event
            const event = new CustomEvent("resetGalleryAnimation", {
              detail: { reset: true },
            });
